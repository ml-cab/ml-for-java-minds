...
- **Top-P** (nucleus sampling) restricts sampling to the smallest set of tokens whose cumulative probability exceeds P. `top_p=0.9` means: keep adding tokens, most likely first, until their probabilities sum past 90%, then sample only from that set. Unlike top-K's fixed count, top-P's candidate set size adapts: when the model is very confident (one token dominates), the set shrinks to just a few tokens; when it's uncertain, the set grows.
