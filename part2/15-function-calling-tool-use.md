...
An LLM cannot query your database, check today's weather, or call a REST API: it can only generate text. **Function calling** (also called tool use) is the pattern that bridges this gap, and it's simpler than it sounds once you see it as structured output ([Chapter 13](#ch-13)) applied to a specific purpose.
