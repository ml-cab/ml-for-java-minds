...
- **llama.cpp**'s server mode (`llama-server`), which documents itself carefully as making no strong claims of full OpenAI compatibility while still implementing enough of the surface, `/v1/chat/completions` and `/v1/embeddings`, that an unmodified OpenAI SDK client works against it by changing only the base URL ([llama.cpp Server README](../rtfms.md#ref-llamacpp-openai-api)).
- **Ollama** (which wraps llama.cpp with model management ergonomics).
- **LocalAI**, a separate Go-based project built from the ground up around this exact idea, describing itself as a drop-in replacement not just for OpenAI but for the Anthropic and Open Responses APIs too, via a small core binary with pluggable, on-demand model backends ([LocalAI Overview](../rtfms.md#ref-localai-openai-api)).
...
In practice, backends differ slightly outside the core contract: model naming conventions, which optional fields they honor versus silently ignore (`seed`, `logit_bias`, and `presence_penalty` are commonly accepted-but-ignored for compatibility rather than actually implemented), and vendor-specific extensions layered on top with a namespaced prefix so they don't collide with the standard fields (Juno's `x_juno_session_id` for stable KV-cache reuse across turns, `x_juno_priority` for request scheduling, and `x_juno_top_k` are examples of this pattern, [Juno Documentation §5.2](../rtfms.md#ref-juno-openai-api)). Not every backend follows that namespacing discipline: llama.cpp's own generation-specific fields (`mirostat`, `repeat_penalty`, and similar) are plain, unprefixed top-level JSON keys rather than an `x_*`-style extension, a real design difference worth noticing rather than a cosmetic one, since an unprefixed field risks silently colliding with a genuinely new official OpenAI field down the line. Design your client code against the documented common subset, and treat vendor extensions as optional, additive capabilities rather than requirements, the same discipline you'd apply to any API you don't fully control.
