...
llama.cpp reaches roughly the same destination by a different route. Rather than merging a Juno-style `.lora` file, its ecosystem typically expects the adapter *already* merged into the base model's full-precision weights (commonly via Hugging Face PEFT, outside llama.cpp entirely) before conversion to GGUF, though llama.cpp also ships `convert_lora_to_gguf.py`, which converts a PEFT adapter into its own separate GGUF adapter format for the load-and-apply-at-runtime path described in [Chapter 24](#ch-24) instead of a merge ([llama.cpp `convert_lora_to_gguf.py`](../rtfms.md#ref-llamacpp-convert-lora)). The upshot: "merge before quantizing" versus "quantize the base once, keep patched tensors at full precision, merge after" aren't just implementation details, they're the difference between needing a separate PEFT-based merge step in Python and merging entirely within the inference engine's own toolchain.
...
*(the `juno merge` command and its `f32-preserve` default policy shown above, [Juno Documentation §4.5](../rtfms.md#ref-juno-merging-adapters))*
