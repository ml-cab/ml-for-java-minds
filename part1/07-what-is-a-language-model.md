...
Fundamental mathematical derivations, accompanied by detailed explanations of the underlying processes, are kindly shared by [Goodfellow, Bengio & Courville (2016)](../references.md#ref-dlbook) in chapter **Deep Feedforward Networks** of Deep Learning book.
...
**Code tokenizes differently from prose.** Indentation, brackets, underscores, and identifiers like `getUserById` split in ways English intuition does not predict — `getUserById` may become `["get", "User", "By", "Id"]`, four tokens for one identifier. JSON, XML, and SQL tend to be token-expensive relative to their information content.
...
Crucially, each token position gets its own contextualised output vector. Self-attention is N independent per-position computations running in parallel — not one shared aggregation — and four tokens in means four separate contextualised vectors out:

```mermaid
flowchart LR
    T1["Token 1"] & T2["Token 2"] & T3["Token 3"] & T4["Token 4"]
 
    T1 & T2 & T3 & T4 --> Q1["Token 1:\nweighted sum over all inputs\n→ Output 1"]
    T1 & T2 & T3 & T4 --> Q2["Token 2:\nweighted sum over all inputs\n→ Output 2"]
    T1 & T2 & T3 & T4 --> Q3["Token 3:\nweighted sum over all inputs\n→ Output 3"]
    T1 & T2 & T3 & T4 --> Q4["Token 4:\nweighted sum over all inputs\n→ Output 4"]
```
...
flowchart LR
        T1["Token 1"] & T2["Token 2"] & T3["Token 3"] & T4["Token 4"]
        T1 & T2 & T3 & T4 --> W["Weighted sum (learned relevance scores)"] --> O["Contextualised output vectors"]
```
...
Because self-attention is a set operation with no inherent notion of order, the paper adds **positional encodings** — vectors that encode each token's position in the sequence — so the model knows that `"France"` at position 4 differs from `"France"` at position 40.
...
The context window is the maximum number of tokens — input plus output combined — the model processes at once. It is strict working memory with a hard cap.
...
**Chat template.** Raw token prediction has no built-in concept of "user" and "assistant" turns; that structure is imposed by wrapping messages in special marker tokens (`<|user|>`, `<|assistant|>`, `<|system|>`, etc.) defined per model family. Use the wrong template and the model's trained behaviors degrade — a frequent real bug when switching model families while reusing prompt code.
...
- **Temperature → 0** — peaks sharpen; the model almost always picks its top token. Consistent and reproducible, but repetitive and formulaic.
